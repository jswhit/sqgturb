...
import sys, time, os
from sqgturb import SQG, rfft2, irfft2, cartdist, \
...
for ntime in range(nassim):
    ...
    # compute forward operator for prior.
    ...
    obsprd_b = (((hxens_b-hxensmean_b)**2).sum(axis=0)/(nanals-1)).mean()
    obfits_b = ((pvob-hxensmean_b)**2).mean()
    obbias_b = (pvob-hxensmean_b).mean()
