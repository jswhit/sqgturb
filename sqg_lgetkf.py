...
import sys, time, os
from sqgturb import SQG, rfft2, irfft2, cartdist, lgetkf_update, gaspcohn
...
kernel_size = int(sys.argv[3]) # Standard deviation for Gaussian kernel (used in ndimage.gaussian_filter) 
if len(sys.argv) > 4:
...
for ntime in range(nassim):
    ...
    #   plt.figure()
    #   plt.imshow(ensmean_wts_smoothed[-1,1,:].reshape(ny,nx))
    #   plt.show()
    #   raise SystemExit
    # apply weights to get increments.
    for n in range(nx*ny):
        for k in range(2):
            xmean[k,n] += np.dot(ensmean_wts_smoothed[:,k,n],xprime2[:,k,n])
            # use orig ens on lhs, mod ens on rhs
            xprime[:,k,n] += np.dot(ensperts_wts_smoothed[:,:,k,n],xprime2[:,k,n])
    xens = xmean + xprime
