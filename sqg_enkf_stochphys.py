...
stochphys_amp = float(sys.argv[4]) # amplitude for additive model forcing

diff_efold = None # use diffusion from climo file

savedata = None # if not None, netcdf filename to save data.
#if stochphys_amp > 0:
#    savedata = 'sqg2_enkf_N256_N64_3hrly_blockmean_stochphys1.nc'
#else:
#    savedata = 'sqg2_enkf_N256_N64_3hrly_blockmean_nostochphys.nc'
...
hcorr = 6.*nc_climo.L/nx
tcorr = 10800.
nsamples = 2
skebs = True
rp = RandomPattern(hcorr,tcorr,nc_climo.L,nx,dt,nsamples=nsamples,stdev=stochphys_amp)
...
if not skebs:
    for nanal in range(nanals):
        ...
        models.append(\
        SQG(pvens[nanal],
        random_pattern=rp.copy(seed=nanal),random_pattern_skebs=None,
    ...
    for nanal in range(nanals):
        ...
        models.append(\
        SQG(pvens[nanal],
        random_pattern_skebs=rp.copy(seed=nanal),random_pattern=None,
        nsq=nc_climo.nsq,f=nc_climo.f,dt=dt,U=nc_climo.U,H=nc_climo.H,\
        r=nc_climo.r,tdiab=nc_climo.tdiab,symmetric=nc_climo.symmetric,\
        diff_order=nc_climo.diff_order,diff_efold=diff_efold,threads=threads))
...
if savedata is not None:
   ...
   nc.stochphys_amp = stochphys_amp
