...
# create random noise
model = SQG(np.zeros((2,N,N),np.float32),nsq=nsq,f=f,U=U,H=H,r=r,tdiab=tdiab,dt=dt,
            diff_order=norder,diff_efold=diff_efold,random_pattern=None,
            dealias=dealias,symmetric=symmetric)
pv = model.pvbar
# add isolated barotropic blob
nexp = 4
x = np.arange(0,2.*np.pi,2.*np.pi/N); y = np.arange(0.,2.*np.pi,2.*np.pi/N)
x,y = np.meshgrid(x,y)
x = x.astype(np.float32); y = y.astype(np.float32)
pv[1] += 2000.*(np.sin(x/2)**(2*nexp)*np.sin(y/2)**(nexp))
...
savedata = 'sqg_N%s_test2.nc' % N # save data plotted in a netcdf file.
