...
tcorr = 18.
hcorr = 2.
rp=RandomPattern(hcorr*L/N,tcorr*dt,L,pv.shape[-1],dt=dt,stdev=stdev,nsamples=2,norm='psi')
model = SQG(pv,nsq=nsq,f=f,U=U,H=H,r=r,tdiab=tdiab,dt=dt,
            diff_order=norder,diff_efold=diff_efold,random_pattern_skebs=rp,
            dealias=dealias,symmetric=symmetric,threads=threads,
            precision=precision)

#  initialize figure.
outputinterval = 43200. # interval between frames in seconds
tmin = 100.*86400. # time to start saving data (in days)
tmax = 300.*86400. # time to stop (in days)
nsteps = int(tmax/outputinterval) # number of time steps to animate
# set number of timesteps to integrate for each call to model.advance
model.timesteps = int(outputinterval/model.dt)
savedata = 'sqg_N%s_skeb.nc' % N # save data plotted in a netcdf file.
