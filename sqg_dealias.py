...
class SQG:
    ...
    def __init__(self,pv,f=1.e-4,nsq=1.e-4,L=20.e6,H=10.e3,U=30.,\
                 ...
                 symmetric=True,dt=None,dealias=False,threads=1):
        ...
        klim = np.array(pi*(2*N/3)/self.L, np.float32)
        if dealias:
            self.dealias = np.where(np.logical_or(np.abs(k)>klim,np.abs(l)>klim),0.,1.)
        else:
            self.dealias = np.ones(k.shape,np.float32)
        ...
        klim = np.array(pi*(2*N/3)/self.L, np.float32)
        if dealias:
            self.dealias = np.where(ktot>klim,0.,1.)
        else:
            self.dealias = np.ones(ktot.shape,np.float32)
        # integrating factor for hyperdiffusion
        # with efolding time scale for diffusion of shortest non-aliased wave (N/3)
        self.hyperdiff =\
        np.exp((-self.dt/self.diff_efold)*(ktot/klim)**self.diff_order)
    ...
    def gettend(self,pvspec=None):
        ...
        dpvspecdt =\
        (1./self.tdiab)*(self.pvspec_eq-pvspec)-\
        self.dealias*rfft2(u*pvx+v*pvy,threads=self.threads)
