...
def enkf_update2(xens,hxens,obs,oberrvar,x,y,xob,yob,levob,hcovlocal_scale,vcovlocal_fact,covl_efold):

    """serial potter method with localization scale dependent on
    expected variance reduction in ob space"""
    ...
    kobs = np.zeros((2,nobs),np.int)
    kobs[1] = 1
    obs = obs.ravel()
    hxmean = hxmean.ravel()
    hxprime = hxprime.reshape((nanals,2*nobs))
    kobs = kobs.ravel()
    oberrs = oberrvar*np.ones(obs.shape, obs.dtype)
    xob = np.tile(xob,2)
    yob = np.tile(yob,2)
    ...
    covl_factmean = 0.
    for nobx in range(len(obs)):

        if covl_efold > 0.:
            # order by decreasing spread
            fsprd = (hxprime**2).sum(axis=0)/(nanals-1)
            fsprd[nobsdone]=0; nob = np.argmax(fsprd)
            nobsdone.append(nob)
            # compute localization length scale based upon predicted
            # variance reduction.
            paoverpb = oberrs[nob]/(fsprd[nob]+oberrs[nob])
            covl_fact = 1. - np.exp( -((1.-paoverpb)/covl_efold) )
        else:
            nob = nobx
            covl_fact = 1.

        covl_factmean = covl_factmean + covl_fact/len(obs)

        fact[:] = 1.; fact[1-kobs[nob]] = vcovlocal_fact

        dist = cartdist(xob[nob],yob[nob],x,y,L,L)
        covlocal = gaspcohn(dist/(hcovlocal_scale*covl_fact)).ravel()
        dist = cartdist(xob[nob],yob[nob],xob,yob,L,L)
        obcovlocal = gaspcohn(dist/(hcovlocal_scale*covl_fact))
        obcovlocal = np.where(kobs != kobs[nob], covl_fact*vcovlocal_fact*obcovlocal, obcovlocal)

        ominusf = obs[nob]-hxmean[nob].copy()
        hxens = hxprime[:,nob].copy().reshape((nanals, 1))
        hpbht = (hxens**2).sum()/(nanals-1)
        gainfact = ((hpbht+oberrs[nob])/hpbht*\
                   (1.-np.sqrt(oberrs[nob]/(hpbht+oberrs[nob]))))
        ...
        for k in range(2):
            pbht = (xprime[:,k,mask].T*hxens[:,0]).sum(axis=1)/float(nanals-1)
            kfgain = fact[k]*covlocal[mask]*pbht/(hpbht+oberrs[nob])
            xmean[k,mask] = xmean[k,mask] + kfgain*ominusf
            xprime[:,k,mask] = xprime[:,k,mask] - gainfact*kfgain*hxens

        # observation space update
        # only update obs within localization radius
        mask = obcovlocal > 1.e-10
        pbht = (hxprime[:,mask].T*hxens[:,0]).sum(axis=1)/float(nanals-1)
        kfgain = obcovlocal[mask]*pbht/(hpbht+oberrs[nob])
