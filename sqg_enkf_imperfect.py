...
print("# hcovlocal=%g vcovlocal=%s diff_efold=%s levob=%s covinflate=%s nanals=%s" %\
     (hcovlocal_scale/1000.,vcovlocal_fact,diff_efold,levob,covinflate,nanals))
...
for ntime in range(nassim):
    ...
    pvprime_b = pvens.copy() - pvensmean_b
    pvsprd_b = ((scalefact*pvprime_b)**2).sum(axis=0)/(nanals-1)
