...
for ntime in range(nassim):
    ...
    for n in range(nx*ny):
        ...
        for nob, ob, oberr in zip(np.arange(nobs_local), obs_local, oberr_local):
            # step 1: update observed variable
            varob = hxprime_local[:,nob]**2; gainob = varob/(varob+oberr)
            hxmean_a = hxmean_local[nob] + gainob*(ob-hxmean_local[nob])
            hxprime_a = np.sqrt(1.-gainob)*hxprime_local[:,nob]

            ominusf = (obs_local[nob] - hxmean_local[nob]).copy()
            hxens = hxprime_local[:, nob].copy()
            hxens2 = hxprime_local2[:, nob].copy()
            ...
            for k in range(2):
                pbht = (xprime[:, k, n].T * hxens2).sum(axis=1) / float(
                    nanals - 1
                )
                kfgain = pbht / (hpbht + oberr)
                xmean[k, n] += kfgain * ominusf
                xprime[:, k, n] -= gainfact * kfgain * hxens[:,np.newaxis]
            # observation space update
            pbht = (hxprime_local[:, mask].T * hxens).sum(axis=1) / float(
                nanals - 1
            )
            kfgain = pbht / (hpbht + oberr)
            hxmean_local += kfgain * ominusf
            hxprime_local  -= gainfact * kfgain * hxens[:,np.newaxis]
