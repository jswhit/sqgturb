...
if len(sys.argv) == 1:
   msg="""
python sqg_enkf.py hcovlocal_scale covinflate assim_thresh vlocal>
    ...
    if local_volume:
        ...
        for n in range(nx*ny):
            ...
            while corrmax > corr_thresh and ncountassim < nobs_local:
                ...
                #    corr = corr + 0.5*np.abs(pbht/np.sqrt(varob*varstate))
                #corr[iassim==1]=0
                ...
                #mask_assim = iassim == 0
                pbht = (hxprime2_local[:,:] * hxprime2_local[:,nobx][:,np.newaxis]).sum(axis=0) / (nanals-1)
                varstate= (hxprime2_local[:,:]**2).sum(axis=0)/(nanals-1)
                corr_ob = np.abs(pbht/np.sqrt(varob*varstate))
                mask_assim = np.logical_and(iassim == 0, corr_ob > corr_thresh)
                hxmean_local[mask_assim] += (pbht[mask_assim]/varob)*obinc_mean
                hxprime_local[:,mask_assim] += (pbht[np.newaxis,mask_assim]/varob)*obinc_prime[:,np.newaxis]
                hxprime2_local[:,mask_assim] += (pbht[np.newaxis,mask_assim]/varob)*obinc_prime2[:,np.newaxis]
                ncountassim += 1

            xens[:,:,n] = xmean[:,n]+xprime[:,:,n]
            nobscount += ncountassim
            #print(nobscount,ncountassim)
        nobscount = nobscount//(nx*ny)
    else:

        paoverpb_thresh = assim_thresh
        hpbht = (scalefact*hxprime2 ** 2).sum(axis=0) / (nanals - 1)
        ...
        while paoverpb_min < paoverpb_thresh and nobscount < nobs:
            ...
            covlocal_state = gaspcohn(dist.ravel()/hcovlocal_scale)
            dist = cartdist(xob[nob],yob[nob],xob,yob,nc_climo.L,nc_climo.L)
            covlocal_ob = gaspcohn(dist/hcovlocal_scale)
            ...
            mask = covlocal_state > 1.0e-10

            for k in range(2):
                pbht = (xprime2[:, k, mask].T * hxens2).sum(axis=1) / float(
                    nanals - 1
                )
                kfgain = covlocal_state[mask] * pbht / (hpbht + oberr)
            ...
            mask = np.logical_and(covlocal_ob > 1.0e-10, iassim == False)
            pbht = (hxprime[:, mask].T * hxens).sum(axis=1) / float(
                nanals - 1
            )
            kfgain = covlocal_ob[mask] * pbht / (hpbht + oberr)
