...
import sys, time, os
from sqgturb import SQG, rfft2, irfft2, cartdist, lgetkf_vloc, gaspcohn
...
ngroups = nanals//2  # number of groups for cross-validation (ngroups=nanals is "leave one out")
...
print("# hcovlocal=%g vcovlocal=%s diff_efold=%s nanals=%s ngroups=%s" %\
     (hcovlocal_km,vcovlocal_fact,diff_efold,nanals,ngroups))
...
for ntime in range(nassim):
    ...
    #raise SystemExit
    pvens2 = pvprime2 + pvensmean # modulated ensemble
    ...
    xens = lgetkf_vloc(xens,xens2,hxens,hxens2,pvob,oberrvar,covlocal_tmp,nanal_index,nerger=nerger,ngroups=ngroups)
